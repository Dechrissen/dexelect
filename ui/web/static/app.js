...
// fetch + JSON-parse with readable failures. Responses generated by nginx
// itself — the /api/generate rate limit (429), or a 502/504 while the service
// restarts — are HTML, so parsing unconditionally would surface a raw
// SyntaxError to the user. Flask's own error responses (400/404) are JSON and
// still parse here, so callers keep reading their `error` field as before.
async function fetchJson(url, options) {
  ...
  try {
    ...
    throw new Error(`HTTP ${r.status} — try again in a moment.`);
  }
}
...
async function loadGame(game) {
  ...
  let data;
  try {
    data = await fetchJson(`/api/game/${encodeURIComponent(game)}`);
  } catch (e) {
    setStatus("Failed to load game data: " + (e.message || e));
    return;
  }
...
async function loadPreset(preset) {
  ...
  try {
    ...
    setStatus("Failed to load preset: " + (e.message || e));
    return;
  }
  renderConfig(data.config);
}
...
async function generate() {
  ...
  let data;
  try {
    data = await fetchJson("/api/generate", {
      method: "POST",
